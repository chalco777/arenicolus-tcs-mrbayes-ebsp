...
meta <- read.table("lagartijas_STRUCTURE.txt",
...
files_bestK <- list.files("results", pattern = paste0("K", bestK, "_rep"), full.names = TRUE)
...
ggplot(pop_cluster, aes(x = factor(AssignedCluster), y = factor(POP_label), fill = n_ind)) +
  geom_tile() +
